...
mn_rst <- signif (mean (commits_rst$slope, na.rm = TRUE), 3)
mod_ros <- summary (lm (commits_ros$slope ~ commits_ros$date))
slope_ros <- signif (mod_ros$coefficients [2, 1], 3)
mn_ros <- signif (mean (commits_ros$slope, na.rm = TRUE), 3)
```

We now repeat the analysis immediately above of relative slopes of non-primary
...
value of `r mn_ros`, with an increase per year of `r slope_ros`. The equivalent
RStudio values are a mean of `r mn_rst` with an increase per year of `r
slope_rst`.

```{r ros-baseline-slope-extra, echo = FALSE}
# precise values to use below
mn_ros <- mean (commits_ros$slope, na.rm = TRUE)
slope_ros <- mod_ros$coefficients [2, 1]
mn_rst <- mean (commits_rst$slope, na.rm = TRUE)
...
`mean_slope` of all), and in that tendency increasing more strongly over time
(`change` = `r signif (res$change[res$category=="image-processing"], 2)`). The
following three categories (`r paste0 (cats [2:4], collapse = ", ")`) all have
relatively low mean values, yet actually become more negative over time,
indicating *decreasing* degrees of community engagement in the code of these
packages. The next category of `r cats [5]` has the highest rate of increase in
engagement over time (`change =` `r signif (res$change[res$category==cats[5]], 2)`).
